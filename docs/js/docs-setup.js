  ...
  "pages": [
    {
      ...
      "keywords": "$destroy $digest $document $http $interval $on $scope $timeout $window _directive1 _directive2 _directive3 achieved admin adminpanelcontroller afin age ajax angularjs annotation api app arguments assets associ attributes au automatiquement aux avec beau bien bonnes bootstrap bower bowererrc build business called camelcase capitaliser cart cas ce cela certains cette change charge classes cleaning clone cloneddd code coherent collisions commande commencer comments commit commun compile complex components comportements composant composants composite compr config configuration contr controller controllers convient correctement cr create creating css dans de defined deploy dernier des develop developp di differentes directive directive1 directive1childdirective directive1service directive2 directive2childdirective directive2doc directive2filter directive3 directives dist docs document documentation documenter doit dom du effectuer elements en encapsulate environnement equipes est etre example exemples existing exposer fa facile factory faire fait fichier fichiers files filter filters fn fois fonts foo function functionality functions git github gulp gulpfile homepage homepagecontroller html https il ils img inattendus inclu install installation interpretation invoked io isolated itesoft itesoft-angular-common jamais javascript jour js json la le les lib librairie libraries lien lieu light link logic loop lowercamelcase main mainctrl maintenable mais mal manipulations md mettre mieux mise model module named naming ne ng ngdoc nom nomm nouveau npm objectif ou overview package panel par partir pas passed permet piping plain plugins plunker pm pmdropdowndirective portant post pour pr pratiques prefer prefixe prefixes prevent prise project projet prompt qu quoi rattach readable reference rendra repertoire reserved retrouv return reusable sass scope scss se sera seront service service1 service2 services setinterval settimeout shopping shoppingcartcontroller showcase single slow soient sont source sous structur structure style substring suivante test tester tests thing third-party travail type ui une uppercamelcase user utilis utiliser vos votre window windows won wrapping"
    },
    {
      "section": "api",
